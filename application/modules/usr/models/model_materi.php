...
class Model_materi extends CI_Model 
{
	...
	public function buat_tabel($data)
	{
        ...
        //heading tabel
        $this->table->set_heading('NO', 'NAMA MATERI', 'TGL. UPLOAD', 'KETERANGAN', 'DOWNLOAD');
        ...
        foreach($data as $row)
        {
            ...
            $this->table->add_row(
                ...
                $row->tgl_upload,
                $row->keterangan,
                anchor('upload-materi/download/'.$row->id_materi, '<i class="fa fa-download"></i> Download', array('class' => 'w3-text-blue w3-hover-text-red')).' '.
                anchor('upload-materi/edit/'.$row->id_materi, '<i class="fa fa-edit"></i> Edit', array('class' => 'w3-text-blue w3-hover-text-red'))
            );
