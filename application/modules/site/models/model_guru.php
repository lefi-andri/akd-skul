...
class Model_guru extends CI_Model 
{
    ...
    {
        $this->db->where('nip', $id)->delete($this->db_tabel);
	...
	public function buat_tabel($data, $start)
	{
        ...
        //heading tabel
        $this->table->set_heading('NO', 'NIP', 'NAMA', 'P/W', 'TEMPAT, TGL. LAHIR', 'ALAMAT', 'TELP.', 'AKSI');
        ...
        foreach($data as $row)
        {
            ...
            $this->table->add_row(
				...
				$row->tlp,
                anchor('guru/edit/'.$row->nip, '<i class="fa fa-edit"></i> Edit', array('class' => 'w3-hover-text-red')).' '.anchor('guru/hapus/'.$row->nip, '<i class="fa fa-trash"></i> Delete', array('class' => 'w3-hover-text-red', 'onclick' => "return confirm('Anda yakin ingin menghapus data ini?')"))
